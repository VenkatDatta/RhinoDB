...
LockManagerA::LockManagerA(deque<Transaction*>* ready_txns) {
    ready_txns_ = ready_txns;
}

bool LockManagerA::WriteLock(Transaction* txn, const Key& key) {
...
bool LockManagerA::ReadLock(Transaction* txn, const Key& key) {
  // Since Part 1A implements ONLY exclusive locks, calls to ReadLock can
  // simply use the same logic as 'WriteLock'.
  return WriteLock(txn, key);
}

void LockManagerA::Release(Transaction* txn, const Key& key) {
  ...
  if (!(tempQueue->size() < 1) && deleteHappened) {
    Transaction *next_transaction = tempQueue->front().txn_;
...
LockMode LockManagerA::Status(const Key& key, vector<Transaction*>* owners) {
...
LockManagerB::LockManagerB(deque<Transaction*>* ready_txns) {
  ready_txns_ = ready_txns;
}

bool LockManagerB::_addLock(LockMode mode, Transaction* txn, const Key& key) {
...
bool LockManagerB::WriteLock(Transaction* txn, const Key& key) {
  return _addLock(EXCLUSIVE, txn, key);
}

bool LockManagerB::ReadLock(Transaction* txn, const Key& key) {
  return _addLock(SHARED, txn, key);
}

void LockManagerB::Release(Transaction* txn, const Key& key) {
...
LockMode LockManagerB::Status(const Key& key, vector<Transaction*>* owners) {
  deque<LockRequest> *dq = _getLockQueue(key);
  if (dq->empty()) {
    return UNLOCKED;
  }
