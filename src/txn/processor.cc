...
void TxnProcessor::NewTxnRequest(Transaction* txn) {
...
Transaction* TxnProcessor::GetTxnResult() {
  Transaction* txn;
...
void TxnProcessor::RunSerialScheduler() {
  ...
  while (tp_.Active()) {
    ...
    if (txn_requests_.Pop(&txn)) {
      ...
      if (txn->getStatus() == COMPLETED_COMMIT) {
        ApplyWrites(txn);
        txn->status_ = COMMITTED;
      } else if (txn->getStatus() == COMPLETED_ABORT) {
        txn->status_ = ABORTED;
      } else {
        // Invalid TxnStatus!
        DIE("Completed Txn has invalid TxnStatus: " << txn->getStatus());
      }
...
void TxnProcessor::RunLockingScheduler() {
  ...
  while (tp_.Active()) {
    ...
    while (ready_txns_.size() > 0) {
      ...
      tp_.RunTask(new Method<TxnProcessor, void, Transaction*>(
            this,
...
void TxnProcessor::ExecuteTxn(Transaction* txn) {
...
void TxnProcessor::ApplyWrites(Transaction* txn) {
  // Write buffered writes out to storage.
  for (map<Key, Value>::iterator it = txn->writes_.begin();
       it != txn->writes_.end(); ++it) {
    storage_->insert(it->first, it->second, txn->unique_id_);
  }
}
 ...
 */
bool TxnProcessor::OCCValidateTransaction(const Transaction &txn) const {
...
void TxnProcessor::RunOCCScheduler() {
  ...
  while (tp_.Active()) {
    ...
    if (txn_requests_.Pop(&txn)) {
      ...
      tp_.RunTask(new Method<TxnProcessor, void, Transaction*>(this, &TxnProcessor::ExecuteTxn,txn));
    }
    ...
    while (completed_txns_.Pop(&finishedTask)) {
      ...
      // Set the status to COMMITTED
        ...
        for (map<Key, Value>::iterator itr = finishedTask->writes_.begin(); itr != finishedTask->writes_.end(); ++itr) {
          storage_->insert(itr->first, itr->second, finishedTask->unique_id_);
        }
...
void TxnProcessor::MVCCExecuteTxn(Transaction* txn) {
  ...
  for (set<Key>::iterator itr = txn->readset_.begin(); itr != txn->readset_.end(); itr++) {
    ...
    //lock before read
    storage_->lock(*itr);
    if (storage_->get(*itr, &result, txn->unique_id_)) {
